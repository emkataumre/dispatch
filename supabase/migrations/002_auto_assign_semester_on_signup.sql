-- Update handle_new_user trigger to auto-assign the currently active semester on signup
CREATE OR REPLACE FUNCTION handle_new_user()
RETURNS trigger AS $$
DECLARE
  active_semester_id uuid;
BEGIN
  SELECT id INTO active_semester_id
  FROM public.semesters
  WHERE start_date <= CURRENT_DATE AND end_date >= CURRENT_DATE
  LIMIT 1;

  IF active_semester_id IS NULL THEN
    RAISE WARNING 'No active semester found for new user %. semester_id will be null.', new.id;
  END IF;

  INSERT INTO public.profiles (id, display_name, semester_id)
  VALUES (new.id, new.raw_user_meta_data->>'display_name', active_semester_id);

  RETURN new;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
